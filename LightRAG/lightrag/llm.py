...
async def zhipu_complete_if_cache(
    ...
    api_key: Optional[str] = "3e4cdb8f677f79641b284e909e0e4712.nuWyOLrJ6FWoWGG0",
...
async def zhipu_embedding(
    texts: list[str], model: str = "embedding-3", api_key: str = "3e4cdb8f677f79641b284e909e0e4712.nuWyOLrJ6FWoWGG0", **kwargs
) -> np.ndarray:
    # dynamically load ZhipuAI
    try:
        from zhipuai import ZhipuAI
    ...
    embedding = response.data[0].embedding  # 获取第一个嵌入
    embedding_array = np.array(embedding)  # 转换为 NumPy 数组
    print(embedding_array.shape)  # 打印嵌入的形状
...
@wrap_embedding_func_with_attrs(embedding_dim=2048, max_token_size=8191)
@retry(
    stop=stop_after_attempt(3),
    wait=wait_exponential(multiplier=1, min=4, max=10),
    retry=retry_if_exception_type((RateLimitError, APIConnectionError, Timeout)),
