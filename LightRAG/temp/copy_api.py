...
from genQA import generate_QA
from genMindMap import create_mind_map_from_key
...
vector_db_config = {
    "自定义知识库": {
        "uri": "https://in05-44377a3e5ebaa5d.serverless.ali-cn-hangzhou.cloud.zilliz.com.cn",
        "user": "db_44377a3e5ebaa5d",
        "password": f"Fz7>|;%F|RQOkQXh",
    ...
    "中学信息技术": {
        "uri": "https://in05-c46940e3965301b.serverless.ali-cn-hangzhou.cloud.zilliz.com.cn",
        "user": "db_c46940e3965301b",
        "password": "Zj4{96go6q*1Zl4^",
        "token": "c6c05bb25c62df3d44a16d47014941d2969c5f42025e53590e1f5be414a5d39915444253892813fce3685ab2665bf379b0f024a7",
...
def init_rag(kb_name):
    # 获取知识库特定的配置
    kb_specific_config = KB_CONFIG.get(kb_name, KB_CONFIG["自定义知识库"])
    ...
    return LightRAG(
        working_dir=os.path.join(LOCAL_STORAGE_DIR, f"{kb_name}"),
...
class InsertText(Resource):
    def post(self):
        ...
        result = executor.submit(rag.insert, texts).result()
...
class GenerateMindMap(Resource):
    def post(self):
        ...
        return {'message': encoded_image, 'code': 200}
