...
def normalize_keypoints(keypoints, center_location, a_idx, b_idx, ignore_value, add_visibility):
    # 确保输入是 TensorFlow tensor 并指定正确的数据类型
    keypoints = tf.cast(keypoints, dtype=tf.float32)
    center_location = tf.cast(center_location, dtype=tf.float32)
    
    # 创建掩码 - 更新为新版本的 TF API
    mask = tf.not_equal(keypoints, tf.cast(ignore_value, dtype=tf.float32))
    
    # 重新定位中心点
    center_expanded = tf.expand_dims(center_location, axis=2)
    keypoints = keypoints - center_expanded
    
    # 计算单位长度 - 使用新版本的 tf.norm
    points_a = tf.gather(keypoints, a_idx, axis=2)
    points_b = tf.gather(keypoints, b_idx, axis=2)
    unit = tf.norm(points_a - points_b, axis=-1, keepdims=True)
    unit = tf.expand_dims(unit, axis=-1)
    
    # 归一化关键点 - 使用 tf.math.divide_no_nan 替代 divide
    keypoints = tf.math.divide_no_nan(keypoints, unit)
    
    # 应用掩码
    keypoints = tf.where(mask, keypoints, tf.cast(ignore_value, dtype=tf.float32))

    # 添加可见性维度
    if add_visibility:
        visibility = tf.cast(mask[:, :, :, :1], dtype=tf.float32)
        keypoints = tf.concat([keypoints, visibility], axis=-1)
